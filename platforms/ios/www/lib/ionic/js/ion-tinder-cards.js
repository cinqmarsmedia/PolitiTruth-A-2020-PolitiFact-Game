(function(ionic) {
    ...
    for(var x = 0; x < transformKeys.length; x++) {
        if(d.style[transformKeys[x]] !== undefined) {
            TRANSFORM_ORIGIN = transformKeys[x];
            break;
        }
    }
    ...
    angular
        .module('ionTinderCards', ['ionic'])
        .directive('tdCard', tdCard)
        .directive('tdCards', tdCards)
        .directive('tdPulse', tdPulse)
        .directive('tdControls', tdControls)
        .directive('tdControlButton', tdControlButton)
        .factory('TDCardDelegate', TDCardDelegate);

    tdCard.$inject = ['$timeout', '$q'];
    tdCards.$inject = ['$rootScope', '$timeout'];
    TDCardDelegate.$inject = ['$rootScope'];

    function tdCard($timeout, $q) {
        ...
        function compile(element, attr) {
            return function($scope, $element, $attr, swipeCards) {
                var el = $element[0];
                var leftText = el.querySelector('.no');
                var rightText = el.querySelector('.yes');
                ...
                var swipeableCard = new SwipeableCardView({
                    ...
                    onSnapBack: function(startX, startY, startRotation) {
                        var leftText = el.querySelector('.yes');
                        var rightText = el.querySelector('.no');
    ...
    function tdCards($rootScope, $timeout) {
        var directive = {
            restrict: 'E',
        ...
        controller.$inject = ['$scope', '$element', '$attrs', 'TDCardDelegate'];

        function controller($scope, $element, $attrs, TDCardDelegate) {
            ...
            this.partial = function(amt) {
                ...
                if(secondCard) bringCardUp(secondCard, amt, 4);
                if(thirdCard) bringCardUp(thirdCard, amt, 8);
            };
        }
    ...
    function tdPulse() {
        var directive = {
            restrict: 'E',
            template: '<div class="td-pulse" ng-transclude></div>',
            transclude: true,
            scope: {}
        };
        return directive;
    ...
    function tdControls() {
        var directive = {
            restrict: 'E',
            template: '<div class="td-buttons" ng-transclude></div>',
            transclude: true,
            scope: {}
        };
        return directive;
    ...
    function tdControlButton() {
        var directive = {
            restrict: 'E',
            template: '<div class="td-button">' +
                         '<button class="button button-clear {{action}}">' +
                            '<i class="icon {{icon}}"></i>' +
                         '</button>' +
                      '</div>',
            transclude: false,
    ...
    function TDCardDelegate($rootScope) {
        var factory = {
            popCard: popCard,
            getSwipeableCard: getSwipeableCard,
        ...
        function popCard($scope, isAnimated) {
            $rootScope.$emit('tdCard.pop', isAnimated);
        }

        function getSwipeableCard($scope) {
        ...
        function _register(handle, instance) {
            instances[handle] = instance;
        }

        function _unregister(handle) {
