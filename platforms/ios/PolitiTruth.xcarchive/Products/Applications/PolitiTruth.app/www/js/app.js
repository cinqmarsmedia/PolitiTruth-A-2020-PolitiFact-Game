...
if ($scope.prevcorr.length==5){
  $scope.openModalpop();
    ...
    $scope.dc = function () {
      $window.open('https://www.cinqmarsmedia.com/devilscalculator/', '_system');
    }
...
$scope.modalthree.hide();
    ...
    $scope.pullnewstories = function () {
      ...
      if (typeof $localStorage.lastpull == 'undefined') {
        $localStorage.lastpull = 0;
      }
      now = new Date();
      //console.log(now.getTime() - lastdate.getTime());

topull = (now.getTime() - lastdate.getTime()) / 259200000;
      if (String(topull) == 'NaN') {
        topull = (now.getTime() - lastdate.getTime()) / 259200000;
      }
// 
      if (topull > 1 && now.getTime()>$localStorage.lastpull+86400000) {
        topull = Math.ceil(topull);

var month=lastdate.getMonth()+1;
if (month==13){month=1}
var date=lastdate.getFullYear()+'-'+month+'-'+lastdate.getDate() 

$localStorage.lastpull=new Date().getTime()

        $.getJSON("https://white-sunset-5f0f.ccj242.workers.dev/corsproxy/?format=json&publication_date_after="+date+"&ordering=publication_date",//
          function (data) {
...
function containsHeadline(obj, list) {
    var i;
    for (i = 0; i < list.length; i++) {
        if (list[i].ruling_headline === obj.ruling_headline) {
            return true;
        }
    }
  ...
  if (!containsHeadline(data[i],$localStorage.newstories) && !containsHeadline(data[i],archive)){
   // console.log('unique');
...
//alert($localStorage.cache)
      if ($localStorage.cache.length > 9) { // interval at which data is posted
        ...
        $http({
          method: 'POST',
          url: 'https://ccj242.com/PolitiFactBackEnd/PoliPost.php?cache=' + $localStorage.cache,
    ...
    $scope.fetchdata=function(){
      ...
      $http({
        method: 'GET',
        url: 'https://ccj242.com/PolitiFactBackEnd/PoliGet.php',
        //data: $scope.outgoingdata,
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
